...
function ThumbnailFacts({ photo, showExtras }: { photo: Photo; showExtras: boolean }) {
  const visible = (showExtras && (photo.stackId || photo.masterId)) || photo.readError
...
function SelectionFrame({ selected, primary }: { selected: boolean; primary: boolean }) {
  let shadow =
    'shadow-[inset_0_0_0_0.5px_rgb(255_255_255/0.1)] group-hover/th:shadow-[inset_0_0_0_1.5px_rgb(255_255_255/0.35)]'
  if (primary) shadow = 'shadow-[inset_0_0_0_2px_var(--color-accent)]'
  else if (selected) {
    shadow = 'shadow-[inset_0_0_0_2px_color-mix(in_srgb,var(--color-accent)_55%,transparent)]'
  }
  return (
    <div
      aria-hidden
  ...
  return (
    ...
    >
      ...
      <ThumbnailFooter
        ...
        scrim={scrim}
      />
      <ThumbnailFacts photo={photo} showExtras={showExtras} />
       ...
       */}
      <SelectionFrame selected={selected} primary={primary} />
    </div>
  )
})
