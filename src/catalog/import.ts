...
import { extOf, isRawFile, isSupported, scanFolder, type ScannedFile } from './fs'
...
export interface ImportProgress {
  phase: 'scanning' | 'reading' | 'done' | 'cancelled'
  total: number
  done: number
  skipped: number
  current: string
...
async function readExif(file: File): Promise<Partial<PhotoMetadata> & { width?: number; height?: number }> {
  try {
    ...
    const e = await exifr.parse(file, {
      ...
      pick: [
        ...
        'FocalLength', 'DateTimeOriginal', 'CreateDate', 'Artist', 'Copyright',
...
export async function importFolder(
...
): Promise<{ folder: CatalogFolder; added: number; skipped: number; failed: number }> {
  const { signal, onProgress, recursive = true } = opts
  const report = (p: Partial<ImportProgress>) =>
    onProgress?.({ phase: 'reading', total: 0, done: 0, skipped: 0, current: '', ...p } as ImportProgress)
  ...
  if (!folder) {
    folder = { id: nextId(), name: handle.name, handle, addedAt: Date.now(), photoCount: 0 }
  ...
  const known = new Set(
    (await db.photos.where('folderId').equals(folder.id).toArray()).map((p) => p.relPath),
  )
  ...
  const workers = Array.from({ length: Math.min(CONCURRENCY, queue.length) }, async () => {
    while (queue.length) {
      ...
      report({ phase: 'reading', total: fresh.length, done, skipped, current: item.name })
      try {
  ...
  return { folder: { ...folder, photoCount }, added, skipped, failed: done - added }
}
...
export async function importFiles(
  ...
  const existing = await db.photos.where('folderId').equals(folder.id).toArray()
  ...
  for (const handle of picked) {
    ...
    if (prior && prior.fileSize === file.size && prior.modifiedAt === file.lastModified) {
      skipped++
      continue
    }
    ...
    for (let n = 2; used.has(relPath); n++) {
      ...
      relPath =
        dot > 0 ? `${handle.name.slice(0, dot)} (${n})${handle.name.slice(dot)}` : `${handle.name} (${n})`
    }
    const item = {
      handle,
      relPath,
    ...
    used.add(relPath)
    scanned.push(item)
    report({ phase: 'scanning', total: scanned.length, skipped, current: handle.name })
  }
  ...
  const workers = Array.from({ length: Math.min(CONCURRENCY, queue.length) }, async () => {
    while (queue.length) {
      if (signal?.aborted) return
      const item = queue.shift()!
      report({ phase: 'reading', total: scanned.length, done, skipped, current: item.name })
      try {
